...
import { OrderEnumSelect, OrderEnumList } from '../OrderEnum'
...
enum OrderDetailComponentState {
	CREATE_INSTANCE,
	UPDATE_INSTANCE,
...
export class OrderDetailComponent implements OnInit {
	...
	ngOnInit(): void {
		...
		if (this.id == 0) {
			this.state = OrderDetailComponentState.CREATE_INSTANCE
		} else {
			if (this.originStruct == undefined) {
				this.state = OrderDetailComponentState.UPDATE_INSTANCE
			} else {
	...
	getOrder(): void {
		...
		this.frontRepoService.pull().subscribe(
			frontRepo => {
				...
				switch (this.state) {
					case OrderDetailComponentState.CREATE_INSTANCE:
	...
	save(): void {
		...
		if (this.order.TargetID == undefined) {
			this.order.TargetID = new NullInt64
		}
		if (this.order.Target != undefined) {
			this.order.TargetID.Int64 = this.order.Target.ID
			this.order.TargetID.Valid = true
		} else {
			this.order.TargetID.Int64 = 0
			this.order.TargetID.Valid = true
		}
