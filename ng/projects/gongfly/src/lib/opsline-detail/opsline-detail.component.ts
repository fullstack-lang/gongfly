...
import { OperationalLineStateEnumSelect, OperationalLineStateEnumList } from '../OperationalLineStateEnum'
...
enum OpsLineDetailComponentState {
	CREATE_INSTANCE,
	UPDATE_INSTANCE,
...
@Component({
	selector: 'app-opsline-detail',
	templateUrl: './opsline-detail.component.html',
	styleUrls: ['./opsline-detail.component.css'],
})
export class OpsLineDetailComponent implements OnInit {
	...
	onChangedActivatedRoute(): void {
		...
		if (this.id == 0) {
			this.state = OpsLineDetailComponentState.CREATE_INSTANCE
		} else {
			if (this.originStruct == undefined) {
				this.state = OpsLineDetailComponentState.UPDATE_INSTANCE
			} else {
		...
		this.OperationalLineStateEnumList = OperationalLineStateEnumList
	}
	...
	getOpsLine(): void {
		...
		this.frontRepoService.pull(this.GONG__StackPath).subscribe(
			frontRepo => {
				...
				switch (this.state) {
					case OpsLineDetailComponentState.CREATE_INSTANCE:
				...
				this.IsTransmittingFormControl.setValue(this.opsline.IsTransmitting)
				this.IsTransmittingBackwardFormControl.setValue(this.opsline.IsTransmittingBackward)
			}
		)
	...
	save(): void {
		...
		this.opsline.IsTransmitting = this.IsTransmittingFormControl.value
		this.opsline.IsTransmittingBackward = this.IsTransmittingBackwardFormControl.value
		if (this.opsline.ScenarioID == undefined) {
			this.opsline.ScenarioID = new NullInt64
		}
		if (this.opsline.Scenario != undefined) {
			this.opsline.ScenarioID.Int64 = this.opsline.Scenario.ID
			this.opsline.ScenarioID.Valid = true
		} else {
			this.opsline.ScenarioID.Int64 = 0
			this.opsline.ScenarioID.Valid = true
		}
	...
	openReverseSelection(AssociatedStruct: string, reverseField: string, selectionMode: string,
		...
		if (selectionMode == SelectionMode.MANY_MANY_ASSOCIATION_MODE) {
			...
			// set up the source
			dialogData.SourceStruct = "OpsLine"
