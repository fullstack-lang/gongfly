...
// SatelliteDetailComponent is initilizaed from different routes
// SatelliteDetailComponentState detail different cases 
enum SatelliteDetailComponentState {
	CREATE_INSTANCE,
	UPDATE_INSTANCE,
...
export class SatelliteDetailComponent implements OnInit {
	...
	state: SatelliteDetailComponentState = SatelliteDetailComponentState.CREATE_INSTANCE
	...
	ngOnInit(): void {
		...
		if (this.id == 0) {
			this.state = SatelliteDetailComponentState.CREATE_INSTANCE
		} else {
			if (this.originStruct == undefined) {
				this.state = SatelliteDetailComponentState.UPDATE_INSTANCE
			} else {
	...
	getSatellite(): void {
		...
		this.frontRepoService.pull().subscribe(
			frontRepo => {
				...
				switch (this.state) {
					case SatelliteDetailComponentState.CREATE_INSTANCE:
						this.satellite = new (SatelliteDB)
						break;
					case SatelliteDetailComponentState.UPDATE_INSTANCE:
						let satellite = frontRepo.Satellites.get(this.id)
						console.assert(satellite != undefined, "missing satellite with id:" + this.id)
	...
	openReverseSelection(AssociatedStruct: string, reverseField: string, selectionMode: string,
		...
		if (selectionMode == SelectionMode.ONE_MANY_ASSOCIATION_MODE) {
			...
			dialogData.ID = this.satellite.ID!
		...
		if (selectionMode == SelectionMode.MANY_MANY_ASSOCIATION_MODE) {
			...
			// set up the source
			dialogData.SourceStruct = "Satellite"
